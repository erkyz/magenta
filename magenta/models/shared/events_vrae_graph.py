...
def build_graph(mode, config, sequence_example_file_paths=None):
  ...
  with tf.Graph().as_default() as graph:
    ...
    encoder_initial_state = cell.zero_state(hparams.batch_size, tf.float32)
    ...
    # mu encoder
    # TODO may want to make this deep, will have to be more robust with state_is_tuple.
    # TODO final state SHOULD be (state size, batch size)
    W_mu_encoder = tf.Variable([TEMP_LATENT_SIZE, encoder_cell.state_size])
    b_mu_encoder = tf.Variable([TEMP_LATENT_SIZE, hparams.batch_size])
    z_mu = tf.matmul(W_mu_encoder, tf.transpose(encoder_final_state)) + b_mu_encoder

    # log var encoder TODO why logvar? may need to play around here
    W_logvar_encoder = tf.Variable([TEMP_LATENT_SIZE, encoder_cell.state_size])
    b_logvar_encoder = tf.Variable([TEMP_LATENT_SIZE, hparams.batch_size])
    z_logvar = tf.matmul(W_logvar_encoder, tf.transpose(encoder_final_state)) + b_logvar_encoder
    ...
    W_z = tf.Variable([decoder_cell.state_size, TEMP_LATENT_SIZE])
    b_z = tf.Variable([decoder_cell.state_size, hparams.batch_size])
    decoder_initial_state = tf.transpose(tf.tanh(tf.matmul(W_z, z) + b_z))

    # inputs: A tensor of shape [batch_size, num_steps, input_size] of floats32s.
    # so we want to just reverse each of the 3rd dimension. correct?
    outputs, final_state = tf.nn.dynamic_rnn(
        encoder_cell, tf.reverse(inputs, [-1]), initial_state=encoder_initial_state, parallel_iterations=1,
        swap_memory=True)
    ...
    if mode == 'train' or mode == 'eval':
      ...
      # "latent loss" -- KL divergence from N(0,I) TODO idk how this was derived exactly. what happened to det and trace?
