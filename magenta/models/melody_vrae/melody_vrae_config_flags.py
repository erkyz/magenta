...
"""Provides a class, defaults, and utils for Melody VRAE model configuration."""
...
class MelodyVraeConfigFlagsException(Exception):
  pass
...
def config_from_flags():
  """Parses flags and returns the appropriate MelodyVraeConfig.

  If `--config` is supplied, returns the matching default MelodyVraeConfig after
  updating the hyperparameters based on `--hparams`.

  If `--melody_encoder_decoder` is supplied, returns a new MelodyVraeConfig using
  ...
  Returns:
    The appropriate MelodyVraeConfig based on the supplied flags.

  Raises:
     MelodyVraeConfigFlagsException: When not exactly one of `--config` or
         `melody_encoder_decoder` is supplied.
  """
  if (FLAGS.melody_encoder_decoder, FLAGS.config).count(None) != 1:
    raise MelodyVraeConfigFlagsException(
  ...
  if FLAGS.melody_encoder_decoder is not None:
    ...
    encoder_decoder = melody_encoder_decoders[FLAGS.melody_encoder_decoder](
        melody_vrae_model.DEFAULT_MIN_NOTE, melody_vrae_model.DEFAULT_MAX_NOTE)
    hparams = magenta.common.HParams()
    hparams.parse(FLAGS.hparams)
    return melody_vrae_model.MelodyVraeConfig(
        generator_details, encoder_decoder, hparams)
  else:
    if FLAGS.config not in melody_vrae_model.default_configs:
      raise MelodyVraeConfigFlagsException(
          '`--config` must be one of %s. Got %s.' % (
              melody_vrae_model.default_configs.keys(), FLAGS.config))
    config = melody_vrae_model.default_configs[FLAGS.config]
